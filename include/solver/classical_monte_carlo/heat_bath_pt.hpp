...
void HeatBathPT(std::vector<SystemType*> *system_list_pointer,
   ...
   for (std::int32_t i = 0; i < system_size; i++) {
      if (max_num_state < (*(*system_list_pointer)[0]).GetNumState(i)) {
         max_num_state = (*(*system_list_pointer)[0]).GetNumState(i);
      }
   }
         ...
         if (a >= b) {
            ...
            for (std::size_t j = 0; j < beta_list.size(); ++j) {
               ...
               for (std::int32_t k = 0; k < system_size; k++) {
                  ...
                  const std::int32_t num_state = (*(*system_list_pointer)[j]).GetNumState(index);
                  double z = 0.0;
                  for (std::int32_t state = 0; state < num_state; ++state) {
                     prob_list[state] = std::exp(-beta_list[j]*(*(*system_list_pointer)[j]).GetEnergyDifference(index, state));
                     z += prob_list[state];
                  }
                  (*(*system_list_pointer)[j]).Flip(index, get_new_state(prob_list, num_state, z, dist_real(random_number_engine)));
               }
            }
            sweep_count--;
         }
