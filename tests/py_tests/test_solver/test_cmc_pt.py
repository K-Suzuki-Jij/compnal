...
from compnal.model.classical import Ising, PolyIsing, ClassicalModelType
...
def test_cmc_pt_ising_chain():
    ...
    temperature_distribution_list = [TemperatureDistribution.ARITHMETIC, TemperatureDistribution.GEOMETRIC]
    ...
    for bc, sum, rne, ssm, td in all_list:
        chain = Chain(
            system_size=5, boundary_condition=bc
        )
        ising = Ising(
        ...
        for i, result in enumerate(result_set):
            ...
            assert result.coordinate_to_index == {(0,): 0, (1,): 1, (2,): 2, (3,): 3, (4,): 4}
            ...
            assert result.model_info.spin_magnitude == {(0,): 0.5, (1,): 0.5, (2,): 0.5, (3,): 0.5, (4,): 0.5}
...
def test_cmc_pt_ising_square():
    ...
    for bc, sum, rne, ssm, td in all_list:
        square = Square(
            x_size=4, y_size=3, boundary_condition=bc
        )
        ising = Ising(
        ...
        for i, result in enumerate(result_set):
            ...
            assert result.energies.shape == (1,)
            assert result.coordinate_to_index == {(i, j): j*4 + i for i in range(4) for j in range(3)}
            assert result.temperature == 0.5 + 0.5 * i

            assert result.model_info.model_type == ClassicalModelType.ISING
            assert result.model_info.interactions == {1: 1.0, 2: -1.0}
            assert result.model_info.spin_magnitude == {(i, j): 1 for i in range(4) for j in range(3)}
...
def test_cmc_pt_ising_cubic():
    ...
    for bc, sum, rne, ssm, td in all_list:
        cubic = Cubic(
            x_size=3, y_size=4, z_size=3, boundary_condition=bc
        )
        ising = Ising(
        ...
        for i, result in enumerate(result_set):
            ...
            assert result.energies.shape == (1,)
            assert result.coordinate_to_index == {(i, j, k): k*12 + j*3 + i for i in range(3) for j in range(4) for k in range(3)}
            assert result.temperature == 0.5 + 0.5 * i

            assert result.model_info.model_type == ClassicalModelType.ISING
            assert result.model_info.interactions == {1: 10.0, 2: -1.0}
            assert result.model_info.spin_magnitude == {(i, j, k): 1 for i in range(3) for j in range(4) for k in range(3)}
            assert result.model_info.spin_scale_factor == 1
...
def test_cmc_pt_ising_infinite_range():
    ...
    for sum, rne, ssm, td in all_list:
        ...
        for i, result in enumerate(result_set):
            ...
            assert result.model_info.model_type == ClassicalModelType.ISING
            assert result.model_info.interactions == {1: 1.0, 2: -1.0}
            assert result.model_info.spin_magnitude == {(0,): 1, (1,): 1, (2,): 1, (3,): 1, (4,): 1}
            assert result.model_info.spin_scale_factor == 1
            assert result.model_info.lattice.lattice_type == LatticeType.INFINITE_RANGE
            assert result.model_info.lattice.system_size == 5
            assert result.model_info.lattice.shape == None
            assert result.model_info.lattice.boundary_condition == BoundaryCondition.NONE
...
def test_cmc_pt_poly_ising_chain():
    ...
    for bc, sum, rne, ssm, td in all_list:
        ...
        ising = PolyIsing(
            lattice=chain, interaction={1: -1, 4: +2}, spin_magnitude=0.5, spin_scale_factor=2
        )
        result_set = CMC.run_parallel_tempering(
            model=ising,
            temperature_range=(0.5, 1.0),
...
def test_cmc_pt_poly_ising_square():
    ...
    for bc, sum, rne, ssm, td in all_list:
        ...
        ising = PolyIsing(
            lattice=square, interaction={1: -1, 3: +2}, spin_magnitude=1, spin_scale_factor=1
        )
        result_set = CMC.run_parallel_tempering(
            model=ising,
            temperature_range=(0.5, 1.0),
...
def test_cmc_pt_poly_ising_cubic():
    ...
    for bc, sum, rne, ssm, td in all_list:
        ...
        ising = PolyIsing(
            lattice=cubic, interaction={1: -1, 3: +2}, spin_magnitude=1, spin_scale_factor=1
        )
        result_set = CMC.run_parallel_tempering(
            model=ising,
            temperature_range=(0.5, 1.0),
...
def test_cmc_pt_poly_ising_infinite_range():
    ...
    for sum, rne, ssm, td in all_list:
        ...
        ising = PolyIsing(
            lattice=infinite_range, interaction={1: 1, 3: -3, 4: +2}, spin_magnitude=1, spin_scale_factor=1
        )
        result_set = CMC.run_parallel_tempering(
            model=ising,
            temperature_range=(0.5, 1.0),
