...
TEST(SolverClassicalMonteCarloSystem, PolyIsingOnInfiniteRangeSpinHalf) {
...
TEST(SolverClassicalMonteCarloSystem, PolyIsingOnInfiniteRangeSpinOne) {
   using InfiniteRange = lattice::InfiniteRange;
   using PolyIsing = model::classical::PolynomialIsing<InfiniteRange>;
   ...
   for (const auto &interaction: interaction_set) {
      InfiniteRange infinite_range{system_size};
      PolyIsing poly_ising{infinite_range, interaction, 1};
      ...
      for (std::int32_t i = 0; i < system_size; ++i) {
         EXPECT_DOUBLE_EQ(system.ExtractSample()(i), -1);
         EXPECT_TRUE(check_include(system.GenerateCandidateState(i), {1, 2}));
      }
