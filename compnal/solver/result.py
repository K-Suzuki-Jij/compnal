    ...
    def export_hdf5(self, path: str) -> None:
        with h5py.File(path, 'w') as f:
            ...
            group.create_dataset(
                'index_to_uuid', 
                data=[str(value) for value in self.index_to_uuid]
            )
            ...
            for key, result in self.results.items():
                ...
                # CMCParams
                group.create_dataset(
                    str(key) + '/params/num_sweeps',
                    data=result.params.num_sweeps
                )
                group.create_dataset(
                    str(key) + '/params/num_samples',
                    data=result.params.num_samples
                )
                group.create_dataset(
                    str(key) + '/params/num_threads',
                    data=result.params.num_threads
                )
                ...
                group.create_dataset(
                    str(key) + '/time/total',
                    data=result.time.total
                )
                group.create_dataset(
                    str(key) + '/time/sample',
                    data=result.time.sample
                )
                group.create_dataset(
                    str(key) + '/time/energy',
                    data=result.time.energy
                )

    @classmethod
    def from_hdf5(cls, path: str) -> None:
        file_contents = h5py.File(path, 'r')
        index_to_uuid = [uuid.UUID(value.decode('utf-8')) for value in file_contents['CMCResultSet/index_to_uuid']]
        new_result_set = cls(
            results={
                universally_unique_id: CMCResult(
                    samples = file_contents['CMCResultSet/results/' + str(universally_unique_id) + '/samples'][...],
                    energies = file_contents['CMCResultSet/results/' + str(universally_unique_id) + '/energies'][...],
                    coordinate_to_index = {
                        tuple(key): value for key, value in zip(
                            file_contents['CMCResultSet/results/' + str(universally_unique_id) + '/coordinate_to_index/keys'], 
                            file_contents['CMCResultSet/results/' + str(universally_unique_id) + '/coordinate_to_index/values']
                        )
                    },
                    temperature = float(file_contents['CMCResultSet/results/' + str(universally_unique_id) + '/temperature'][...]),
                    model_info = ClassicalModelInfo(
                        model_type=ClassicalModelType(file_contents['CMCResultSet/results/' + str(universally_unique_id) + '/model_info/model_type'].asstr()[...]),
                        interactions={
                            key: value for key, value in zip(
                                file_contents['CMCResultSet/results/' + str(universally_unique_id) + '/model_info/interactions/keys'], 
                                file_contents['CMCResultSet/results/' + str(universally_unique_id) + '/model_info/interactions/values']
                            )
                        },
                        spin_magnitude={
                            tuple(key): value for key, value in zip(
                                file_contents['CMCResultSet/results/' + str(universally_unique_id) + '/model_info/spin_magnitude/keys'], 
                                file_contents['CMCResultSet/results/' + str(universally_unique_id) + '/model_info/spin_magnitude/values']
                            )
                        },
                        spin_scale_factor=int(file_contents['CMCResultSet/results/' + str(universally_unique_id) + '/model_info/spin_scale_factor'][...]),
                        lattice=LatticeInfo(
                            lattice_type=LatticeType(file_contents['CMCResultSet/results/' + str(universally_unique_id) + '/model_info/lattice/lattice_type'].asstr()[...]),
                            system_size=int(file_contents['CMCResultSet/results/' + str(universally_unique_id) + '/model_info/lattice/system_size'][...]),
                            shape=tuple(file_contents['CMCResultSet/results/' + str(universally_unique_id) + '/model_info/lattice/shape']),
                            boundary_condition=BoundaryCondition(file_contents['CMCResultSet/results/' + str(universally_unique_id) + '/model_info/lattice/boundary_condition'].asstr()[...])
                        )
                    ),
                    params=CMCParams(
                        num_sweeps=int(file_contents['CMCResultSet/results/' + str(universally_unique_id) + '/params/num_sweeps'][...]),
                        num_samples=int(file_contents['CMCResultSet/results/' + str(universally_unique_id) + '/params/num_samples'][...]),
                        num_threads=int(file_contents['CMCResultSet/results/' + str(universally_unique_id) + '/params/num_threads'][...]),
                        num_replicas=int(file_contents['CMCResultSet/results/' + str(universally_unique_id) + '/params/num_replicas'][...]) if 'num_replicas' in file_contents['CMCResultSet/results/' + str(universally_unique_id) + '/params'] else None,
                        num_replica_exchange=int(file_contents['CMCResultSet/results/' + str(universally_unique_id) + '/params/num_replica_exchange'][...]) if 'num_replica_exchange' in file_contents['CMCResultSet/results/' + str(universally_unique_id) + '/params'] else None,
                        state_update_method=StateUpdateMethod(file_contents['CMCResultSet/results/' + str(universally_unique_id) + '/params/state_update_method'].asstr()[...]),
                        random_number_engine=RandomNumberEngine(file_contents['CMCResultSet/results/' + str(universally_unique_id) + '/params/random_number_engine'].asstr()[...]),
                        spin_selection_method=SpinSelectionMethod(file_contents['CMCResultSet/results/' + str(universally_unique_id) + '/params/spin_selection_method'].asstr()[...]),
                        algorithm=CMCAlgorithm(file_contents['CMCResultSet/results/' + str(universally_unique_id) + '/params/algorithm'].asstr()[...]),
                        seed=int(file_contents['CMCResultSet/results/' + str(universally_unique_id) + '/params/seed'][...])
                    ),
                    hardware_info=CMCHardwareInfo(
                        cpu_threads=int(file_contents['CMCResultSet/results/' + str(universally_unique_id) + '/hardware_info/cpu_threads'][...]),
                        cpu_cores=int(file_contents['CMCResultSet/results/' + str(universally_unique_id) + '/hardware_info/cpu_cores'][...]),
                        cpu_name=str(file_contents['CMCResultSet/results/' + str(universally_unique_id) + '/hardware_info/cpu_name'].asstr()[...]),
                        memory_size=float(file_contents['CMCResultSet/results/' + str(universally_unique_id) + '/hardware_info/memory_size'][...]),
                        os_info=str(file_contents['CMCResultSet/results/' + str(universally_unique_id) + '/hardware_info/os_info'].asstr()[...])
                    ),
                    time=CMCTime(
                        date=str(file_contents['CMCResultSet/results/' + str(universally_unique_id) + '/time/date'].asstr()[...]),
                        total=float(file_contents['CMCResultSet/results/' + str(universally_unique_id) + '/time/total'][...]),
                        sample=float(file_contents['CMCResultSet/results/' + str(universally_unique_id) + '/time/sample'][...]),
                        energy=float(file_contents['CMCResultSet/results/' + str(universally_unique_id) + '/time/energy'][...])
                    )
